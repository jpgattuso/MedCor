...
This script adds the following variables to the tab "GPS Coordinates":
...
* mi_parbottom10: maximum monthly PAR at 10 m depth in mol/(m2 d) calculated as min_parbottom10 = par * exp(-kdpar * 10)
...
geo <- geo %>% 
  dplyr::mutate(id = seq(1:nrow(geo)),
                #latitude = str_replace_all(latitude, " ", ""), # remove white space
                #latitude = str_replace_all(latitude, "″", "\""), # replace "
                #longitude = str_replace_all(longitude, " ", ""), # remove white space
                #longitude = str_replace_all(longitude, "″", "\""), # replace "
...
# save
write_csv(x = geo3, file = "data/par.csv")
writexl::write_xlsx(x = geo3, path = "data/par.xlsx")
```
